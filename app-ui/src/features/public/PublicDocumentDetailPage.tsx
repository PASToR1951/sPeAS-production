...
function CompiledChildCard({ child }: { child: LooseRecord }) {
  ...
  return <article className="peas-document-child-card"><FileText aria-hidden="true" /><div className="peas-document-child-card__body"><div className="peas-document-child-card__heading"><div><span className="peas-document-child-card__category">{category}</span><h3>{title}</h3></div></div><span className="peas-document-child-card__section-label">Metadata</span><dl className="peas-document-child-card__metadata" aria-label={`${title} metadata`}><div><dt>Authors</dt><dd>{authors.join(", ") || "Unknown author"}</dd></div><div><dt>Publication date</dt><dd>{publicationDateOf(child)}</dd></div>{child.pages ? <div><dt>Pages</dt><dd>{String(child.pages)}</dd></div> : null}</dl><div className="peas-document-child-card__abstract"><span className="peas-document-child-card__section-label">Abstract</span>{abstract ? <p id={abstractId} className={!expanded && canExpandAbstract ? "is-collapsed" : ""}>{abstract}</p> : <p className="is-muted">No abstract is available for this paper.</p>}{canExpandAbstract ? <button type="button" aria-controls={abstractId} aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}>{expanded ? "Show less" : "Show full abstract"}</button> : null}</div><ChildTermGroup label="Topics" values={classification.topics} parameter="topic" /><ChildTermGroup label="Keywords" values={classification.keywords} parameter="keyword" /></div><a className="peas-document-child-card__details" aria-label={`View details for ${title}`} href={`/pages/guest-single.html?id=${encodeURIComponent(childId)}`}>View</a></article>;
}

function ChildTermGroup({ label, values, parameter }: { label: string; values: Array<{ id?: number; name: string }>; parameter: "topic" | "keyword" }) {
