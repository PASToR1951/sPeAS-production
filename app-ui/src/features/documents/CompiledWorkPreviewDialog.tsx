...
import { BookOpen, CheckCircle2, ChevronDown, Download, ExternalLink, FileText, FolderOpen, ListTree, RefreshCw, Search, XCircle } from "lucide-react";
...
import { compiledForewordUrl, compiledStudyPdfUrl, fetchCompiledPreviewManifest, type CompiledPreviewManifest, type CompiledPreviewStudy } from "../../lib/api/compiled-documents";
...
export function CompiledWorkPreviewDialog({ document, onOpenChange }: { document: DocumentRecord | null; onOpenChange: (open: boolean) => void }) {
  ...
  const selectedTitle = selection.kind === "foreword" ? "Collection foreword" : selectedStudy?.title ?? "";
  const selectedPdf = selection.kind === "foreword"
    ? Boolean(manifest?.collection.hasForeword)
    : Boolean(selectedStudy?.hasPdf);
  const selectedPdfUrl = selection.kind === "foreword"
    ? document ? compiledForewordUrl(document.id, "inline") : ""
    : selectedStudy ? compiledStudyPdfUrl(selectedStudy.id, "inline") : "";
  const selectedDownloadUrl = selection.kind === "foreword"
    ? document ? compiledForewordUrl(document.id, "attachment") : ""
    : selectedStudy ? compiledStudyPdfUrl(selectedStudy.id, "attachment") : "";
  const activeTabId = selection.kind === "overview" ? "compiled-preview-tab-overview" : selection.kind === "foreword" ? "compiled-preview-tab-foreword" : `compiled-preview-tab-study-${selection.studyId}`;
        ...
        {loading ? <CompiledPreviewLoading /> : error ? <CompiledPreviewError message={sessionExpired ? "Your administrator session has expired. Close this dialog and sign in again." : error} onRetry={loadManifest} sessionExpired={sessionExpired} onClose={() => onOpenChange(false)} /> : manifest ? (
          <div className="peas-compiled-preview-dialog__body">
            ...
            <section id="compiled-preview-panel" className="peas-compiled-preview-pane" role="tabpanel" aria-labelledby={activeTabId} aria-label={selectedTitle || "Collection overview"} aria-live="polite">
              {selection.kind === "overview" ? <CollectionOverview manifest={manifest} /> : selection.kind === "foreword" ? <PdfSelectionPane key={`foreword-${document?.id ?? ""}`} title="Collection foreword" description="The foreword for this compiled publication." pdf={selectedPdf} url={selectedPdfUrl} downloadUrl={selectedDownloadUrl} /> : selectedStudy ? <StudySelectionPane key={selectedStudy.id} study={selectedStudy} url={selectedPdfUrl} downloadUrl={selectedDownloadUrl} /> : <CollectionOverview manifest={manifest} />}
            </section>
          </div>
        ) : null}
      </DialogContent>
...
function PdfSelectionPane({ title, description, pdf, url, downloadUrl, abstract }: { title: string; description: string; pdf: boolean; url: string; downloadUrl: string; abstract?: string | null }) {
  const [loaded, setLoaded] = useState(false);
  const [failed, setFailed] = useState(false);
  const [failureKind, setFailureKind] = useState<PdfReaderError | null>(null);
  const [readerKey, setReaderKey] = useState(0);
  useEffect(() => { setLoaded(false); setFailed(false); setFailureKind(null); setReaderKey((value) => value + 1); }, [url, pdf]);
