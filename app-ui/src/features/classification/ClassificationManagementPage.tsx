...
export function ClassificationManagementPage() {
  ...
  return <main className="peas-admin-island peas-classification-management">
    ...
    <div className="peas-classification-summary" aria-label="Classification integrity summary">
      <div><strong>{agendas.filter((agenda) => agenda.isActive).length}</strong><span>active official agendas</span></div>
    ...
    <section className="peas-classification-management__section">
      ...
      <div className="peas-classification-list">{visibleAgendas.length ? visibleAgendas.map((agenda) => { const index = agendas.findIndex((item) => item.id === agenda.id); return <div className="peas-classification-row peas-classification-agenda-row" key={agenda.id}><span className="peas-classification-row__order">{String(index + 1).padStart(2, "0")}</span><div className="peas-classification-agenda-row__details"><strong>{agenda.name}</strong></div><span className="peas-classification-agenda-row__usage">{agenda.documentCount} {agenda.documentCount === 1 ? "document" : "documents"}</span><Badge tone={agenda.isActive ? "green" : "slate"}>{agenda.isActive ? "Active" : "Retired"}</Badge><div className="peas-classification-row__actions"><Button size="sm" variant="outline" aria-label={`Edit ${agenda.name}`} onClick={() => openAgendaEdit(agenda)}><Pencil aria-hidden="true" /> Edit</Button><Button size="sm" variant="outline" aria-label={`Move ${agenda.name} up`} disabled={busy || index === 0} onClick={() => void moveAgenda(agenda, -1)}><ArrowUp aria-hidden="true" /></Button><Button size="sm" variant="outline" aria-label={`Move ${agenda.name} down`} disabled={busy || index === agendas.length - 1} onClick={() => void moveAgenda(agenda, 1)}><ArrowDown aria-hidden="true" /></Button><Button size="sm" variant="outline" onClick={() => agenda.isActive ? setAgendaRetireTarget(agenda) : void toggleAgenda(agenda)}>{agenda.isActive ? "Retire" : "Reactivate"}</Button></div></div>; }) : <p>No agendas match this filter.</p>}</div>
    </section>
    ...
    <Dialog open={Boolean(agendaEdit)} onOpenChange={(open) => { if (!open) setAgendaEdit(null); }}>
      <DialogContent>
        <DialogHeader><DialogTitle>Edit research agenda</DialogTitle><DialogDescription>Update the agenda name. Its internal identifier is generated automatically and remains stable for existing documents.</DialogDescription></DialogHeader>
    ...
    <AlertDialog open={Boolean(agendaRetireTarget)} onOpenChange={(open) => { if (!open) setAgendaRetireTarget(null); }}>
      <AlertDialogContent>
        <AlertDialogHeader><AlertDialogTitle>Retire this research agenda?</AlertDialogTitle><AlertDialogDescription>“{agendaRetireTarget?.name}” is linked to {agendaRetireTarget?.documentCount ?? 0} {agendaRetireTarget?.documentCount === 1 ? "document" : "documents"}. Existing links will remain intact, but the agenda will no longer be available for new uploads.</AlertDialogDescription></AlertDialogHeader>
