// The registry component is JavaScript-first; keep its shader code intact while
// exposing it safely from this TypeScript application.
// @ts-nocheck
...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      container.dataset.renderer = 'gradient-fallback';
      return;
    }
    ...
    let isMotionAllowed = !motionPreference.matches;
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && isMotionAllowed && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
    ...
    const onMotionPreference = event => {
      isMotionAllowed = !event.matches;
      if (isMotionAllowed) tryStart();
      else {
        tryStop();
